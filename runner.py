...
class Runner():
    ...
    def dsm_train(self, opt, stage, num_itr, batch_x, batch_t):
        ...
        with self.ema_f.average_parameters():
            ...
            compute_xs_label = sde.get_xs_label_computer(opt, self.ts, A)
            if opt.forward_net.startswith('Linear'):
                D = torch.eye(A.shape[0]) - 2 * A
                print(util.green(f'D\n{D}'))
                lambda_v = self.z_f.net.Lambda.detach()
                D_lambda_beta_max = (1-2*lambda_v)*opt.beta_max
                print(util.green(f'A Lambda {lambda_v[0]:.3f}, {lambda_v[1]:.3f}'))
                print(util.green(f'D Lambda beta max {D_lambda_beta_max[0]:.3f}, {D_lambda_beta_max[1]:.3f}'))
                print(util.green(f'U {self.z_f.net.U.weight.detach()}'))
                print(util.green(f'V {self.z_f.net.V.weight.detach()}'))
                self.prior_covariance = sde.compute_variance(torch.Tensor([1.0]), D, opt.beta_min, opt.beta_max, opt.beta_r)[0][0]
        ...
        for it in range(num_itr):
            ...
            self.log_dsm_train(opt, stage, it, loss, optimizer, num_itr)
    ...
    def dsm_train_stage(self, opt, stage):
        self.dsm_train(
            opt, stage, num_itr=opt.num_itr_dsm, batch_x=opt.train_bs_x_dsm, batch_t=opt.train_bs_t_dsm,
        )
                ...
                for z in [self.z_f, self.z_b]:
                    ...
                    if opt.forward_net.startswith('Linear'):
                        ...
                        if z.direction == 'backward':
                            ...
                            if 1:
                                for nfe in [opt.nfe, opt.interval]:
                                    gap = opt.interval // nfe
                                    if gap * nfe != opt.interval:
                                        continue
                                    assert gap >= 1
                                    custom_ts = self.ts[::gap].repeat(gap).sort().values
                                    xs_ode, _, _ = self.dyn.sample_traj(custom_ts, z, save_traj=True, adaptive_prior=adaptive_prior, policy_f=z_f)
                                    if nfe == opt.interval:
                                        fn = "stage{}-{}-ode".format(stage, z.direction)
                                    else:
                                        fn = "stage{}-{}-ode-nfe-{}".format(stage, z.direction, nfe)
    ...
    def _print_train_itr(self, opt, stage, it, loss, optimizer, num_itr, name):
        time_elapsed = util.get_time(time.time()-self.start_time)
        lr = optimizer.param_groups[0]['lr']
        print("[{0}] stage {1}/{2} train_it {3}/{4} | lr:{5} | loss:{6} | time:{7}"
            .format(
                util.magenta(name),
    ...
    def log_dsm_train(self, opt, stage, it, loss, optimizer, num_itr):
        if it % 20 != 0:
            return
        self._print_train_itr(opt, stage, it, loss, optimizer, num_itr, name='DSM backward')
        if opt.log_tb:
